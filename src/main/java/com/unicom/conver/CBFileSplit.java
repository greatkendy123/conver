...
public class CBFileSplit {
	 ...
	 * 构造方法
	 */
	public CBFileSplit(String currentPath) {
		super();
	...
	public static void printInfo() {
		...
		System.out.println("生成CBSS文件的线程数:"+CREATE_FILE_THREAD_SIZE);
		System.out.println("每个线程处理的CBSS文件个数:"+HANDLE_FILE_SIZE);
		System.out.println("当前匹配到的CBSS大文件数量:"+bigFileSzie);
		System.out.println("=====================================================================================");
		System.out.println();
	}
	/**
	...
	public static void handleCBSS(String outPath) throws Exception{
		...
		for(int x=0; x<bigFileSzie; x++) {
			...
			String firstRow = contentAllList.get(0);
			List<String> contentDataList = contentAllList.subList(1, contentAllList.size());
			int allSize = contentAllList.size();
			int size = contentDataList.size();//allSize-1
			...
			for(int i=0; i<count; i++) {
				from = i * (FILE_LINES+1);
				to = (i+1) * (FILE_LINES+1)-1;
				if(to > allSize ) to = size;
				//System.out.println("起："+from+" 止："+to);
				
				//构造CB文件所需元素
				List<String> subDataList = contentDataList.subList(from, to);
		...
		System.out.println("CBSS主线程执行完成！");
	}
	...
	public static String getFirstRowString(String _firstRow,int index) {
		...
		_firstRow = _firstRow.substring(0, _firstRow.length()-4);
	...
	static class CBSSFileCreator implements Runnable{
		...
		public void run(){
			try {
				...
				System.out.println("生成cbss文件,耗时："+(end-start)+",第"+atomicInteger2.getAndIncrement());
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
