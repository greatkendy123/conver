 ...
 */
public class VacConvert {

	//文件队列容量(目前是1500个文件）
	private static final Integer MAX_FILE_QUEUE_SIZE = 3000;
	
	//处理VAC文件内容的线程数
	private static final Integer MAX_THREAD_SIZE = 4;
	 ...
	 */
	public VacConvert(String _currentPath) {
		super();
	...
	public static void handleVAC(String _outPath) {
		try {
			...
			VacConvert vacConver = new VacConvert(_outPath);
			...
			vacConver.quotaTask();//为线程池中的线程分配任务
	...
	public void quotaTask() {
		...
		for(int i=0;i<fileSiz;i++) {
			try {
				...
				create_poor.submit(new FileCreator(creat_file_queue.take()));
	...
	static class FileFormator implements Runnable{
		...
		public void run(){
			try {
				...
				System.out.println("新文件队列："+creat_file_queue.size());
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
		}
	...
	static class FileCreator implements Runnable{
		...
		public void run(){
			try {
				...
				System.out.println("create cbss file ,cost "+(end-start)+", index:"+atomicInteger.getAndIncrement());
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
